...
export default class Critters {
  ...
  constructor(options) {
    this.options = Object.assign(
      { logLevel: 'info', externalStylesheets: [] },
      options || {}
    );
    ...
    if (this.options.external !== false) {
      ...
      await Promise.all(
        externalSheets.map((link) =>
          this.embedLinkedStylesheet(link, compilation, outputPath, publicPath)
        )
      );
    }
...
// Handle error when parsing stylesheet
// Most of our partner's CSS fails parsing
// JSDOM css parsing fails even when css.parse works
